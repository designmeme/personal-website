...
subtitle:
excerpt: 업비트 웹사이트의 로그인 연장 버튼을 자동으로 클릭하는 북마클릿의 사용법과 동작 원리를 설명한다.
...
<div className="lead">
데스크톱에서 업비트 차트를 장시간 열어 두면 로그인이 만료된다. 수동 연장 주기가 30분이므로 반복해서 연장 버튼을 눌러야 한다.
</div>

로그인 연장 버튼을 자동으로 클릭하는 자바스크립트 코드를 작성했다.
이 코드는 북마클릿으로 실행한다.
이 프로젝트는 <FontAwesomeIcon icon={faGithub} /> [upbit-web-login-forever](https://github.com/designmeme/upbit-web-login-forever)에서 확인할 수 있다.

---
## 북마클릿 사용하기

1. 아래 북마클릿을 브라우저의 즐겨찾기(북마크)로 등록한다.
    ```
    ...
    ![북마클릭을 북마크로 추가한다.](../images/post/add-upbit-login-bookmarklet.png)

2. 업비트 웹사이트에 로그인 후 1)에서 등록한 즐겨찾기를 클릭한다.

    ![북마클릭을 클릭한다.](/images/post/click-upbit-login-bookmarklet.gif)

3. 화면 상단에 `자동연장 사용중(0)` 영역이 보인다. 마우스를 올리면 툴팁으로 안내사항이 보인다.

    ![마우스를 올리면 툴팁으로 안내사항이 보인다.](/images/post/hover-upbit-login-div.gif)

4. 로그인 '연장' 버튼이 생기면 자동으로 클릭되어 로그인이 연장되고 화면에 연장 횟수가 `자동연장 사용중(1)` 처럼 변경된다. 웹사이트의 언어 설정(한국어, 영어)에 관계없이 모두 작동한다.

    ![자바스크립트가 연장 버튼을 자동으로 클릭한다.](/images/post/extended-upbit-login.gif)

---
## 그만 사용하기

1. 사용 중인 탭/창에서 화면을 새로고침한다.
2. 사용 중인 탭/창을 닫는다.

---
## 코드 설명

북마클릿의 원본 [소스코드(main.js)](https://github.com/designmeme/upbit-web-login-forever/blob/main/main.js)는 다음과 같다.
...
* **1**번 줄: 북마크를 클릭 했을 때 활성화된 웹사이트의 주소가 `upbit.com`인 경우만 동작한다.
* **4~15**번 줄: 자동연장 기능을 사용 하고 있다는 것을 알 수 있게 **사용중 안내 요소**를 만들어서 사이트 헤더에 추가한다.
* **17~24**번 줄: 10분 마다 로그인 **연장(Extend)** 버튼이 있는지 확인하고 있다면 클릭해서 로그인을 연장한다. 참고로 로그인 연장 버튼은 잔여시간이 30분 남은 시점부터 표시되고 있다.
* **21~22**번 줄: 자동으로 로그인을 연장했다면 **연장 횟수**가 그에 맞춰 변경된다.

참고로 원본 코드를 [북마클릿 생성기](https://mrcoles.com/bookmarklet/)를 사용해 북마클릿 코드로 변환했다.
